...
namespace DirectUI 
{
	...
	private:
		...
		unsigned char*			m_pGifData;           // 为LoadGIFFirstFrame使用，保存gif数据
		UINT					m_nGifSize;          // 为LoadGIFFirstFrame使用，记录GIF文件大小
		BOOL					m_bOnlyFirstFrame;	//为LoadGIFFirstFrame使用，当前只load了1帧，要播放的时候才load所有的
		UINT					m_nGifCount;//为LoadGIFFirstFrame使用,记录帧数
		UINT					m_nGifMask;//为LoadGIFFirstFrame使用,记录mask
	};
	...
	public:
		...
		bool IsAutoPlayEnabled(){return m_bAutoPlayEnabled;}
		void SetAutoPlayEnabled(bool bEnable){m_bAutoPlayEnabled=bEnable;}
	...
	private:
		void CheckAutoPlay();
	protected:
		bool		m_bMousePlayEnabled;//鼠标移上去时自动播放，移出来停止播放
		bool		m_bPlaying;
		bool		m_bAutoPlayEnabled;//自动播放
		DWORD		m_dwIndex;
		INotifyUI*	m_pNotify;
	};
}
