...
namespace DirectUI
{
	...
	public:
		...
		bool Move(int nSrcIndex, int nDesIndex);
...
#ifdef _UNICODE
		const CUIString& operator=(LPCSTR lpStr);
		const CUIString& operator+=(LPCSTR lpStr);
#else
	...
	class DirectUI_API CUIStringA
	{
	public:
		enum { MAX_LOCAL_STRING_LEN = MAX_PATH };

		CUIStringA();
		CUIStringA(const char ch);
		CUIStringA(const CUIStringA& src);
		CUIStringA(LPCSTR lpsz, int nLen = -1);
		~CUIStringA();
		...
		void Append(LPCSTR pstr);
		void Assign(LPCSTR pstr, int nLength = -1);
		...
		const CUIStringA& operator=(const CUIStringA& src);
		const CUIStringA& operator=(const char ch);
		const CUIStringA& operator=(LPCSTR pstr);

		CUIStringA operator+(const CUIStringA& src) const;
		CUIStringA operator+(LPCSTR pstr) const;
		const CUIStringA& operator+=(const CUIStringA& src);
		const CUIStringA& operator+=(LPCSTR pstr);
		const CUIStringA& operator+=(const char ch);
		...
		int Compare(LPCSTR pstr) const;
		int CompareNoCase(LPCSTR pstr) const;

		void MakeUpper();
		void MakeLower();

		CUIStringA Left(int nLength) const;
		CUIStringA Mid(int iPos, int nLength = -1) const;
		CUIStringA Right(int nLength) const;
